...
// Enqueues the job when Redis is available; falls back to synchronous processing.
exports.generateSummary = asyncHandler(async (req, res) => {
  ...
  // Persist transcript first so the worker can resolve it
  ...
  if (job) {
    // Queue is available — respond immediately with 202 and the job id
...
// Enqueues when Redis available; falls back to synchronous.
...
exports.generateMinutes = asyncHandler(async (req, res) => {
  ...
  // Persist transcript for the worker
  await AIResult.findOneAndUpdate(
    { meetingId: req.params.meetingId },
    { transcript, meetingId: req.params.meetingId },
    { upsert: true, new: true }
