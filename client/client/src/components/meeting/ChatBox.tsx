    ...
    <div className="flex flex-col h-full" role="log" aria-label="Meeting chat" aria-live="polite">
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
        ...
        {messages.map((msg, i) => {
          ...
          const showName = !isOwn && msg.senderName !== prevMsg?.senderName;
          return (
            <div key={msg.id} className={clsx('flex gap-2', isOwn && 'flex-row-reverse')}>
              {/* Avatar — only show on first msg in group */}
              {showName || isOwn ? (
                <div
              ...
              <div className={clsx('max-w-[75%] flex flex-col gap-0.5', isOwn && 'items-end')}>
                {showName && (
                  <span className="text-[10px] font-semibold text-[#64748B] px-1">{msg.senderName}</span>
                )}
                <div className={clsx(
                ...
                <span className="text-[10px] text-[var(--color-text-dim)] px-1" aria-label={`Sent at ${fmt(msg.timestamp)}`}>
                  {fmt(msg.timestamp)}
                </span>
              </div>
            </div>
      ...
      <div className="p-3 border-t border-[var(--color-border)]">
        ...
        >
          ...
          <button
            type="submit"
            disabled={!input.trim()}
            className="p-2 rounded-lg bg-[var(--color-primary)] text-white hover:bg-[var(--color-primary-hover)] disabled:opacity-40 transition-all focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-1"
