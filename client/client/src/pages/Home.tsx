...
/* ─── helpers ─── */
const W = 'max-w-[1320px] mx-auto px-6 md:px-10 xl:px-16';
        ...
        <div className={`${W} h-20 flex items-center justify-between`}>
          ...
          <Link to={ROUTES.HOME} className="flex items-center gap-2.5 group">
            <div className="relative w-10 h-10 rounded-2xl flex items-center justify-center overflow-hidden"
              ...
              <Zap size={18} className="text-white fill-current relative z-10" />
...
const Banner = () => {
  ...
  if (!v) return null;
  return (
    <div className="relative z-[101] flex items-center justify-center gap-3 px-5 py-2.5 text-[12.5px] font-semibold text-white"
      style={{ background: 'linear-gradient(90deg,#1E3A8A,#4F46E5,#7C3AED,#4F46E5,#1E3A8A)', backgroundSize: '300% 100%', animation: 'gradientShift 6s ease-in-out infinite' }}>
      <Sparkles size={13} className="text-yellow-300 shrink-0" />
      <span>✨ Introducing <strong>IntelliMate AI v4</strong> — Real-time meeting intelligence</span>
      <a href="#ai-solutions" className="ml-1 underline underline-offset-2 text-white/80 hover:text-white font-bold">Explore →</a>
      <button onClick={() => setV(false)} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/40 hover:text-white transition-colors"><X size={13} /></button>
    </div>
  );
};
...
/* ─── Logo strip ─── */
const LogoStrip = () => (
  <section className="py-14 relative overflow-hidden border-t border-white/[0.05]" style={{ background: '#050C1F' }}>
    <p className="text-center text-[11px] font-bold text-white/20 uppercase tracking-[0.25em] mb-8">Trusted by world-class teams</p>
    <div className="relative">
      <div className="flex gap-16 animate-marquee whitespace-nowrap">
        {['Microsoft', 'Google', 'Stripe', 'Atlassian', 'Salesforce', 'AWS', 'Oracle', 'SAP', 'Notion', 'Linear',
          'Microsoft', 'Google', 'Stripe', 'Atlassian', 'Salesforce', 'AWS', 'Oracle', 'SAP', 'Notion', 'Linear'].map((name, i) => (
          <span key={i} className="text-[14px] font-black text-white/12 uppercase tracking-[0.22em] hover:text-white/35 transition-colors cursor-default shrink-0">{name}</span>
        ))}
      </div>
      <div className="absolute inset-y-0 left-0 w-32 pointer-events-none" style={{ background: 'linear-gradient(90deg,#040918,transparent)' }} />
      <div className="absolute inset-y-0 right-0 w-32 pointer-events-none" style={{ background: 'linear-gradient(270deg,#040918,transparent)' }} />
    </div>
  </section>
);
...
/* ─── Products ─── */
const cards = [
  { title: 'AI Transcription', desc: 'Real-time, speaker-aware transcription with 99% accuracy.', icon: Mic, color: '#6366F1', glow: 'rgba(99,102,241,0.25)' },
  { title: 'Smart Summaries', desc: 'Automatic summaries with key insights and action items.', icon: Sparkles, color: '#10B981', glow: 'rgba(16,185,129,0.25)' },
  { title: 'Action Items', desc: 'Extract and assign action items with deadlines.', icon: CheckSquare, color: '#F59E0B', glow: 'rgba(245,158,11,0.25)' },
  { title: 'Enterprise Security', desc: 'End-to-end encryption with HIPAA/SOC2 compliance.', icon: Shield, color: '#EC4899', glow: 'rgba(236,72,153,0.25)' },
  { title: 'Collaboration', desc: 'Real-time comments, mentions, and team workspace.', icon: Users, color: '#3B82F6', glow: 'rgba(59,130,241,0.25)' },
  { title: 'Integrations', desc: 'Connect with Slack, Notion, Jira, and 50+ tools.', icon: Globe, color: '#A855F7', glow: 'rgba(168,85,247,0.25)' },
];

const Products = () => (
  <section id="products" className="py-32 lg:py-40" style={{ background: '#050C1F' }}>
    <div className={W}>
        {/* Heading — centered */}
  ...
  /* ─── Stats ─── */
const Stats = () => (
  <section id="stats" className="py-32 lg:py-40"
    style={{ background: 'linear-gradient(135deg,#071440 0%,#12318f 50%,#4f1d96 100%)' }}>
    ...
    <div className={W}>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-16 lg:gap-24">
        {[
          { end: 50, suffix: 'M+', label: 'Meetings Hosted', icon: Video },
          { end: 99.99, suffix: '%', label: 'Uptime SLA', icon: Award, decimals: 2 },
          { end: 150, suffix: '+', label: 'Countries', icon: Globe },
          { end: 10000, suffix: '+', label: 'Enterprise Clients', icon: Users },
        ].map((s, i) => (
          <motion.div key={s.label} {...FU(i * 0.08)} className="text-center group">
            ...
            <div className="text-[12px] font-semibold text-white/40 uppercase tracking-widest">{s.label}</div>
          </motion.div>
        ))}
      </div>
    </div>
...
/* ─── AI Section ─── */
const AISection = () => (
  <section id="ai-solutions" className="py-32 lg:py-40" style={{ background: '#050C1F' }}>
    <div className={W}>
...
/* ─── Security ─── */
const Security = () => (
  <section id="enterprise" className="py-32 lg:py-40" style={{ background: '#050C1F' }}>
    <div className={W}>
      <div className="relative rounded-[2rem] overflow-hidden p-12 md:p-20 text-center"
        ...
        <motion.div {...FU()} className="relative z-10 max-w-3xl mx-auto">
          ...
          <h2 className="font-black text-white tracking-tight mb-7" style={{ fontSize: 'clamp(2rem,4vw,3.5rem)', letterSpacing: '-0.03em' }}>
...
/* ─── Testimonials ─── */
const Testimonials = () => (
  <section id="testimonials" className="py-32 lg:py-40" style={{ background: '#040918' }}>
    <div className={W}>
      <motion.div {...FU()} className="text-center mb-16">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-amber-500/20 bg-amber-500/[0.07] text-[11px] font-bold text-amber-400 uppercase tracking-[0.12em] mb-6">
          <Star size={10} className="fill-current" />Customer Stories
        </div>
        <h2 className="font-black text-white tracking-tight" style={{ fontSize: 'clamp(2rem,3.5vw,3rem)', letterSpacing: '-0.03em' }}>
          Loved by the industry's finest.
        </h2>
      </motion.div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          { name: 'Alex Rivera', role: 'CTO', company: 'TechFlow Inc', quote: 'The AI summaries alone save our engineering leads 5+ hours every single week. IntellMeet completely transformed how we run syncs — we\'ll never go back.', avatar: 'AR', color: 'from-indigo-500 to-violet-600', rating: 5 },
          { name: 'Sarah Chen', role: 'VP Product', company: 'Nexus Labs', quote: 'Finally a video tool that actively helps us ship. The Linear integration is flawless, the AI notes are genuinely useful, and the UI is gorgeous.', avatar: 'SC', color: 'from-rose-500 to-pink-600', rating: 5 },
          { name: 'Marcus Thorne', role: 'Head of Operations', company: 'ScaleUp HQ', quote: 'Enterprise security that doesn\'t slow us down. SSO took 10 minutes to configure, E2E encryption gives our legal team peace of mind. Best-in-class.', avatar: 'MT', color: 'from-emerald-500 to-teal-600', rating: 5 },
        ].map((t, i) => (
          <motion.div key={i} {...FU(i * 0.09)}
            className="flex flex-col justify-between p-7 rounded-3xl border transition-all duration-300 hover:-translate-y-1.5"
            style={{ background: 'rgba(255,255,255,0.025)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.07)' }}
            whileHover={{ borderColor: 'rgba(255,255,255,0.14)', boxShadow: '0 20px 60px rgba(0,0,0,0.4)' }}>
            <div>
              <div className="flex gap-0.5 mb-5">
                {[...Array(t.rating)].map((_, j) => <Star key={j} size={13} className="fill-amber-400 text-amber-400" />)}
              </div>
              <p className="text-[15px] text-white/62 leading-relaxed mb-7">"{t.quote}"</p>
            </div>
            <div className="flex items-center gap-3 pt-5 border-t border-white/[0.07]">
              <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${t.color} flex items-center justify-center text-[11px] font-black text-white shrink-0`}>
                {t.avatar}
              </div>
              <div>
                <p className="text-[13px] font-bold text-white">{t.name}</p>
                <p className="text-[11px] text-white/30 mt-0.5">{t.role} · {t.company}</p>
              </div>
            </div>
          </motion.div>
        ))}
...
/* ─── CTA ─── */
const CTA = () => (
  <section id="cta" className="py-32 lg:py-40" style={{ background: '#040918' }}>
    <div className={W}>
      <motion.div {...FU()}
        className="relative rounded-[2rem] p-14 lg:p-24 text-center overflow-hidden"
        ...
        <div className="relative z-10 max-w-2xl mx-auto">
          <h2 className="font-black text-white tracking-tight mb-6" style={{ fontSize: 'clamp(2rem,4.5vw,3.75rem)', letterSpacing: '-0.04em' }}>
