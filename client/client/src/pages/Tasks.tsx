    ...
    >
      <div className="flex items-start gap-2">
        ...
        <button
          ...
          className="opacity-0 group-hover:opacity-100 p-0.5 text-[var(--color-text-dim)] hover:text-[var(--color-danger)] transition-all"
        >
          <Trash2 size={12} />
        </button>
      </div>
  ...
  const handleDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    if (overCol) {
      ...
      if (activeTask.status === overTask.status) {
        ...
        // no backend call needed for ordering (order stored client-side);
        // if your API supports order fields, fire updateMutation here.
      } else {
  ...
  if (isLoading) return (
    <div className="flex flex-col gap-8 h-full">
      ...
      <div className="grid grid-cols-3 gap-6 flex-1">
        {[0, 1, 2].map(i => <div key={i} className="bg-white/5 rounded-2xl h-96 animate-pulse" />)}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-5 animate-fade-in h-full">
      ...
      >
            ...
            return (
              <div key={col.id} className="flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-auto text-xs bg-[var(--color-surface-2)] text-[var(--color-text-muted)] rounded-full px-2 py-0.5">
                    {colTasks.length}
                  </span>
                </div>
                ...
                >
                  <div
                    id={col.id}
                    className="kanban-col flex flex-col gap-2 bg-[var(--color-surface)]/50 rounded-xl p-2 border border-dashed border-[var(--color-border)] min-h-[200px]"
                  >
                    {colTasks.map(task => (
                      <TaskCard
                        key={task._id}
        ...
        <DragOverlay>
          {activeTask && (
            <div className="bg-[var(--color-surface-2)] border border-[var(--color-primary)]/50 rounded-xl p-3 text-sm font-medium text-[var(--color-text)] shadow-2xl rotate-1">
              {activeTask.title}
            </div>
          )}
        </DragOverlay>
