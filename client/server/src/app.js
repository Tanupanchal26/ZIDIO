...
// Session — ONLY for OAuth state (short-lived, never used for auth tokens)
// secure and sameSite are driven by isProd to prevent insecure cookies in production
app.use(session({
  secret:            config.jwt.secret,
  resave:            false,
  saveUninitialized: false,
  cookie: {
    secure:   config.isProd,                    // was hardcoded false — now correct
    httpOnly: true,
    sameSite: config.isProd ? 'strict' : 'lax', // strict in prod, lax in dev
